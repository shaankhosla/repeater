...
use crate::card::Card;
use crate::fsrs::{
    LEARN_AHEAD_THRESHOLD_MINS, Performance, ReviewStatus, ReviewedPerformance, update_performance,
};

pub(super) async fn get_card_performance_with(
    ...
    let review_count: i64 = row.try_get("review_count")?;
    if review_count == 0 {
    ...
    Ok(Performance::Reviewed(ReviewedPerformance {
        last_reviewed_at: row
            .try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("last_reviewed_at")?
            .ok_or_else(|| anyhow!("missing last_reviewed_at for card {card_hash}"))?,
        stability: row
            .try_get::<Option<f64>, _>("stability")?
            .ok_or_else(|| anyhow!("missing stability for card {card_hash}"))?,
        difficulty: row
            .try_get::<Option<f64>, _>("difficulty")?
            .ok_or_else(|| anyhow!("missing difficulty for card {card_hash}"))?,
        interval_raw: row
            .try_get::<Option<f64>, _>("interval_raw")?
            .ok_or_else(|| anyhow!("missing interval_raw for card {card_hash}"))?,
        interval_days: row
            .try_get::<Option<i64>, _>("interval_days")?
            .ok_or_else(|| anyhow!("missing interval_days for card {card_hash}"))?
            as usize,
        due_date: row
            .try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("due_date")?
            .ok_or_else(|| anyhow!("missing due_date for card {card_hash}"))?,
        review_count: review_count as usize,
    }))
}

pub(super) async fn write_card_performance_with(
    connection: &mut SqliteConnection,
    card_hash: &str,
    performance: ReviewedPerformance,
) -> Result<()> {
    let interval_days = performance.interval_days as i64;
    let review_count = performance.review_count as i64;
    ...
    .bind(performance.last_reviewed_at)
    .bind(performance.stability)
    .bind(performance.difficulty)
    .bind(performance.interval_raw)
    .bind(interval_days)
    .bind(performance.due_date)
    .bind(review_count)
    .bind(card_hash)
    ...
    if result.rows_affected() != 1 {
        return Err(anyhow!("card {card_hash} does not exist"));
    }
    Ok(())
}
...
impl DB {
    ...
    pub async fn update_card_performance(
        ...
        let current_performance =
            get_card_performance_with(&mut transaction, &card.card_hash).await?;
        let reviewed_at = optional_now.unwrap_or_else(chrono::Utc::now);
        let new_performance =
            update_performance(current_performance, review_status, reviewed_at, retention)?;
        write_card_performance_with(&mut transaction, &card.card_hash, new_performance).await?;
    ...
    pub async fn get_card_performance(&self, card: &Card) -> Result<Performance> {
        ...
        get_card_performance_with(&mut connection, &card.card_hash).await
    }
